...
import scrapy
import json
from scrapy import responsetypes,Item,Field
...
class CGSpider(scrapy.Spider):
    ...
    def parse_school(self, response):
        data =  response.json() # Newer version of Scrapy come with shortcut to get JSON data
        for result in data['id_producto']:
            for index in range(len(result)):
                items = ProductoItem()
                items['producto'] = result[index]['nombre']
                items['precio'] = result[index]['precioEspecial']
                items['pnEan'] = result[index]['codigo_principal']
                items['id'] = result[index]['id_producto']
                yield items
    #def parse_school(self, response):
    #    data = response.json() # Newer version of Scrapy come with shortcut to get JSON data
    #    yield {
